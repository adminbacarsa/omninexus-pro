...
export async function listPlazosFijo(filtros?: { inversorId?: string; estado?: string }): Promise<PlazoFijo[]> {
  try {
    let q = query(collection(getDb(), COL), orderBy('fechaInicio', 'desc'));
...
export async function createPlazoFijo(
  ...
  try {
    const createFn = httpsCallable<
      ...
      { id: string }
    >(getFunctionsInstance(), 'createPlazoFijo');
  ...
  try {
    ...
    const fechasRef = collection(getDb(), COL, pfId, SUB_FECHAS);
    ...
    if (movId) {
      try {
        const movSnap = await getDoc(doc(getDb(), 'movimientos_fondo', movId));
        if (movSnap.exists()) {
...
export async function listMovimientosPlazoFijo(pfId: string): Promise<MovimientoPlazoFijo[]> {
  const ref = collection(getDb(), COL, pfId, SUB_MOV);
...
export async function listFechasPago(pfId: string): Promise<FechaPagoPF[]> {
  const ref = collection(getDb(), COL, pfId, SUB_FECHAS);
  try {
...
export async function pagarInteres(
  ...
  const fechasRef = doc(getDb(), COL, pfId, SUB_FECHAS, fechaPagoId);
  ...
  await updateDoc(doc(getDb(), COL, pfId, SUB_MOV, movId), {
...
export async function cancelarPlazoFijo(
  ...
  for (const f of fechas) {
    if ((f.estado === 'pendiente' || f.estado === 'vencido') && f.id) {
      const fechaRef = doc(getDb(), COL, pfId, SUB_FECHAS, f.id);
